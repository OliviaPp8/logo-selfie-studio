...
const PreviewSection = ({ photo, company }: PreviewSectionProps) => {
  ...
  return (
    <div className="w-full">
      ...
      <div className="relative rounded-xl overflow-hidden border-2 border-border bg-gradient-to-br from-muted/50 to-muted min-h-[400px] flex items-center justify-center">
        {hasContent ? (
          <div className="relative w-full h-full min-h-[400px] flex items-end justify-center p-8 gap-4">
            {/* User Photo */}
            <div className="relative z-10 flex-shrink-0">
              <img
                src={photo}
                alt="Your photo"
                className="h-80 w-auto object-contain rounded-lg shadow-card"
              />
            </div>
            
            {/* Company Logo */}
            <div 
              className="relative z-10 flex-shrink-0 p-6 bg-card/90 backdrop-blur-sm rounded-2xl shadow-card animate-float"
            >
              <img
                src={companyLogos[company.id]}
                alt={`${company.name} logo`}
                className="h-20 w-auto object-contain"
                style={{ filter: company.id === "apple" ? "invert(0)" : "none" }}
              />
            </div>
